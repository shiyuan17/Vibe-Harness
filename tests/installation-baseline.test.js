...
async function runCli(args) {
  const { stdout } = await execFileAsync(process.execPath, [cliPath, ...args], {
    cwd: rootDir,
    env: { ...process.env, VIBE_HARNESS_TEST_OFFLINE: '1' },
  });
...
test('MVP install previews then writes one reusable project baseline backup', async () => {
  const target = await mkdtemp(path.join(tmpdir(), 'vibe-harness-install-baseline-'));
  try {
    ...
    const stateAfterFirst = JSON.parse(await readFile(path.join(target, '.vibe-harness', 'install-state.json'), 'utf8'));
    ...
    const stateAfterSecond = JSON.parse(await readFile(path.join(target, '.vibe-harness', 'install-state.json'), 'utf8'));
    ...
    assert.match(repeatedRule.backup, /^\.vibe-harness\/backups\//u);
...
test('baseline backup failure happens before any install target is changed', async () => {
  const target = await mkdtemp(path.join(tmpdir(), 'vibe-harness-install-baseline-failure-'));
  try {
    await runCli(['init', '--project', target]);
...
test('write install rejects project links that redirect managed files outside the project', async () => {
  const target = await mkdtemp(path.join(tmpdir(), 'vibe-harness-install-link-project-'));
  const outside = await mkdtemp(path.join(tmpdir(), 'vibe-harness-install-link-outside-'));
  try {
    ...
    await assert.rejects(
      ...
      ], { cwd: rootDir, env: { ...process.env, VIBE_HARNESS_TEST_OFFLINE: '1' } }),
...
test('rollback rejects project links before deleting managed files outside the project', async () => {
  const target = await mkdtemp(path.join(tmpdir(), 'vibe-harness-rollback-link-project-'));
  const outside = await mkdtemp(path.join(tmpdir(), 'vibe-harness-rollback-link-outside-'));
  try {
    await runCli(['init', '--project', target, '--target', 'codex', '--profile', 'core']);
