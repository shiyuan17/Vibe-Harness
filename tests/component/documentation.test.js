...
test('read-only evaluation keeps Memory body access behind recovery and authorization', async () => {
  ...
  assert.match(agents, /专项 Skill 限制 Memory 证据边界时，只确认路径存在与元数据，不读正文/u);
  ...
  assert.match(installPlanner, /仅当需要恢复项目状态且已获授权时读 Memory body/u);
  assert.match(installPlanner, /专项 Skill 限制 Memory 证据边界时，只确认路径存在与元数据，不读正文/u);
});
