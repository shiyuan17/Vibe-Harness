...
Triage 是团队收件箱：进入 Triage 的 Issue 默认不进入常规视图，必须经 accept / duplicate / decline / snooze 之一处置后才算进入工作流。Agent 只读解释流程，不自动处置 Triage Issue。
...
label group / project / team，用于驱动 Triage Rules 自动路由。不确定时留空交由 triager 判断。
...
- Assignee：通常留给 triager，不强制报告者填写。
