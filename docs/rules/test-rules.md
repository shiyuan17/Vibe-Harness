...
记录实际命令、退出码和关键结果。检查有效执行且断言不符时，标记为产品或行为失败；工具缺失、权限不足、网络失败、超时或环境异常导致检查未有效执行时，标记为**验证受阻（degraded）**，不得推断产品通过或失败。失败不得通过降低断言绕过；验证受阻时缩小完成主张，并记录失败阶段、替代证据、未验证行为和剩余风险。
