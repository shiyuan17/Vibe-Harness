...
- 完整 mode 与 effect 枚举以 `governance-core.md` Execution Envelope 条款为准；本清单是 Git 域不含 `linearWrite` 的子集。
...
- 提交主题使用 `<type>(<scope>): <描述>`，常用类型为 feat、fix、docs、refactor、test、chore 和 eval。
...
- 默认分支名使用 `<type>/<short-topic>`；已有任务分支或用户指定分支优先。
...
full profile 会安装项目级 pre-commit 和 pre-push 文件，但不会修改本地或全局 Git 配置。是否启用 `core.hooksPath` 由用户决定。客户端 Hook 可被本地用户绕过，强制策略应放在 CI 和服务端保护中。
