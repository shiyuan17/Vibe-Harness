...
- 开发、调试和临时运行日志写入 `.cognis/log/`，例如 `vite-dev.out/err`；`.cognis/artifacts/` 只用于截图、trace、导出文件等交付证据产物。
- 排障前先定位日志来源和检索方式，再按明确时间范围、级别、组件、关联 ID 或错误码收敛。
...
- 检索：提供按明确时间范围、level、component、关联 ID、error code 和关键事件过滤的命令或查询。
...
- 留存：说明开发机、测试、CI 和生产环境的日志保留范围，以及权限不足时的升级路径。
...
1. 记录症状、时间窗口、环境、用户可见错误、前一轮变更和复现步骤。
