...
角色人格与领域 Skill 正交：当前原子动作只激活一个角色人格，并按需加载 description 匹配的互补领域 Skill。角色改变决策视角，不改变授权、安全边界或证据要求。
...
- 从 .agents/roles/<role-id>.md 读取当前角色 Prompt，不预加载全部角色正文；未投影角色目录时（例如只安装规则、未启用角色的仓库）读取源角色 Prompt `roles/prompts/<role-id>.md`，不得据此判定角色不可用。
- 固定权限子 Agent 在职责变化、缺少输入、工具或权限时回传父 Agent；它不得靠切换角色扩大权限，也不得擅自再派子 Agent。
- 仅在用户明确请求协作，或独立并行、二次复审有明确收益且宿主允许时，才创建真实子 Agent；授权和工作边界须明确。
