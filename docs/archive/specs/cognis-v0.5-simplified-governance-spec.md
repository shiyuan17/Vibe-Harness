...
- 复杂输入先整理为目标、范围、约束、验收标准和待决策项；简单请求不重复或强制套模板。
- 输出按信息形态选择最小结构：可跟踪事项使用 checkbox todo，顺序步骤使用有序列表，并列要点使用无序列表，对比/重复字段/证据使用 Markdown 表格，单个状态/决策/风险使用短信息块。
- 代码、日志、JSON、路径和命令输出保留 fenced block 与原始字面量；用户明确指定格式时优先服从用户格式。
- 信息块不得依赖平台专属卡片语法，不使用装饰性或嵌套卡片；终端不支持时退化为短标题和键值列表。
