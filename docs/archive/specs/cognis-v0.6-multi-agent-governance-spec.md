...
- 所有任务沿用治理内核规定的固定拼接顺序：稳定前缀、动态后缀、当前请求。稳定前缀只包含治理规则、专项规则、profile/工具能力、adapter 能力和规范指纹。
- parent 可以复用稳定治理前缀；child 只接收完成其写入范围所需的最小、稳定的缓存上下文和最小输入，不继承无关任务历史，也不得把动态证据写回稳定前缀。
- fan-in 后必须在目标工作区刷新动态上下文并复验实际 diff；不得复用合并前 child 的工作区状态、命令输出或完成证据。
- 任务切换、规范变更、adapter 变更、profile 变更和新证据产生都会使相应缓存层失效。规范指纹用于一致性和失效判断，不改变现有 `governanceHash` 的语义，不新增运行时字段或持久化提示正文。
- 无法提供 prefix cache 的 adapter 按完整提示发送，不能因此改变任务合同、证据门禁或安全边界。
