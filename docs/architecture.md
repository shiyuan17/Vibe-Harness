...
- full：全安装，在 core 上增加项目内工具 runtime、codebase 初始索引、MCP 注册、agentmemory skill、`.agents/memory/` 本地回退库和 Codex hooks；稳定 provision 排除 Agentmemory，显式 `--allow-preview` 后才能安装该依赖面。
