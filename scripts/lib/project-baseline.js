...
const baselineTarget = '.loopengine/baseline.json';
const reportTarget = 'docs/loopengine/PROJECT_BASELINE.md';
...
function createRecommendations({ profile, tools, verification, vcs }) {
  ...
  for (const [name, command] of Object.entries(verification.commands)) {
    if (command.status === 'failed' || command.status === 'blocked') {
      ...
      recommendations.push({ code: `VERIFY_${name.toUpperCase()}_${command.status.toUpperCase()}`, message: `在 loopengine.config.json 中补齐或确认 ${name} 验证命令。`, priority: 'P1' });
    }
  }
  ...
  if (verification.mode === 'static') {
    recommendations.push({ code: 'BASELINE_VERIFICATION_NOT_RUN', command: 'loopengine baseline --project <project> --verify --write', message: '执行项目验证并把结果纳入基线。', priority: 'P2' });
  }
...
async function readPreviousBaseline(targetDir, installState) {
...
function renderReport(baseline) {
  ...
  return `<!-- Generated by LoopEngine. Refresh with loopengine baseline; do not edit this managed file. -->
# 项目基线
...
export async function createProjectBaseline({
  ...
  const previous = await readPreviousBaseline(targetDir, installState);
...
export async function collectProjectBaselineInputs({
  ...
  if (
    ...
    throw baselineError('BASELINE_INSTALL_INVALID', 'Project installation is missing or inconsistent; run loopengine install and validate before baseline.');
  }
