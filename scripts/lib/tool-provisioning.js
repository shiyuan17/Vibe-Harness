...
async function runToolPhases(spec, commandRunner, env, targetDir, signal) {
  ...
  for (const spec of plan) {
    ...
    try {
      if (effectiveCommandRunner) {
        tools[spec.id] = await runToolPhases(spec, effectiveCommandRunner, env, targetDir, signal);
        ...
        tools[spec.id] = await runToolPhases(spec, defaultPhaseRunner, env, targetDir, signal);
      }
    } catch (error) {
