...
export async function runVerificationPlan({
  ...
  return {
    ...
    verification: {
      ...
      deferredChecks,
      deferredMicroChecks: (plan.deferredMicroChecks ?? []).map((item) => ({ ...item })),
      selectedMicroChecks: (plan.selectedMicroChecks ?? []).map((item) => ({ ...item })),
      minimumTier: plan.minimumTier ?? null,
      escalation: plan.escalation ?? { required: deferredChecks.length > 0, reason: deferredChecks.length > 0 ? 'deferred evidence remains' : null, nextTier },
      executionTier: plan.executionTier ?? null,
      riskLevel: plan.riskLevel,
      planMode: plan.planMode,
      scope: plan.scope ?? 'layer',
      scopeConfidence: plan.scopeConfidence ?? 'unknown',
      budgetMs: plan.budgetMs ?? null,
      estimatedCostMs: plan.estimatedCostMs ?? null,
      environment: plan.environment ?? { mode: 'cold', fallback: 'blocked' },
