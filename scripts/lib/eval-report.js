...
export function renderEvalReport(model) {
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="icon" href="data:,"><title>Cognis Online Eval 决策报告</title><style>
...
</style></head><body><header class="top"><h1>Cognis Online Eval 决策报告</h1><p>发布结论、证据状态与工程诊断分层展示。效率和趋势指标仅用于诊断，不形成隐式门禁。</p><div class="evidence"><span class="badge ${allPassed ? 'good' : 'bad'}">本次运行：${allPassed ? '通过' : '未通过'}</span><span class="badge ${referenceReady ? 'good' : 'partial'}">Reference：${referenceReady ? '已匹配' : '未建立 / 不匹配'}</span><span class="badge ${qualityPartial ? 'partial' : 'good'}">数据质量：${qualityPartial ? '部分可信' : '完整'}</span></div></header>
