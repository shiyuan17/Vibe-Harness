...
export const CONTENT_QUALITY_CHECKS = [
  ...
  },
  {
    ...
    terms: [
      ...
      '阻塞项',
      '下一步动作',
      '锚点语义以 `governance-core.md` 为唯一规范来源',
    ],
  },
  {
    ...
    terms: [
      ...
      'Verification receipts record the actual command or manual criterion with its result',
      'Implementation units (long tasks track them in the state anchor)',
      'Decisions made',
      'Blockers',
      'Next action',
      'Anchor semantics are owned by `governance-core.md`',
    ],
  },
  {
    file: 'templates/finding-report.md',
    terms: [
      '占位外壳',
      '会写错数据或绕过控制',
      '口径、关联或控件明确错误',
  ...
  {
    file: 'templates/workflow.en-US.md',
    terms: [
      'Recommended steps, not a gate',
      'state anchor',
      'Exit criteria',
      'New session',
      'Fan-out',
      'Stop',
      'write path stays single-agent by default',
    ],
  },
  {
  ...
  },
  {
    ...
    terms: [
      ...
      'WCAG 2.2 AA',
      '负控',
      // 长命令与收据复用：墙钟预算与运行器超时双口径，复用前提与失效条件。
      '长命令与收据复用',
      'Agent 墙钟预算',
      '项目未声明上限时参考 10 分钟',
      '超过上限按 blocked 处理',
      ...
      '一用例一个失败原因',
      '表驱动用例必须参数化命名',
      // 测试金字塔与成本分层：规模分布、同步/异步阻塞范围、端到端准入与深度异步收据。
      '测试金字塔',
      '成本分层',
      '单元测试是主体',
      '端到端只保留核心业务主链路和关键风险流程',
      '失败阻塞当前实施单元',
      '失败阻塞合并或完成声明',
      '失败阻塞集成、发布或依赖该证据的完成声明',
      '完整聚合入口',
      'E2E 准入',
      '默认每个关键旅程保留 1–3 条',
      '软预算不超过活跃用例的 10%',
  ...
  {
    file: 'docs/rules/review-report.md',
    terms: [
      // 定位：交付物契约，不是流程门或完成门禁。
      '不新增评审流程或完成门禁',
      '不构成流程门或完成门禁',
