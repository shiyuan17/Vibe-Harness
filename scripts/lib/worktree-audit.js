...
export function validateWorktrees(value, options = {}) {
  ...
  for (const task of tasks) {
    ...
    if (entry !== undefined && integration) {
      if (integration.integrated === false) {
        push(
...
function finishAudit({ entries, problems, tasks }, options, extra = {}) {
  ...
  return {
    ...
    // git-rules.md §Worktree forbids cleanup before merge-back.
    cleanupAllowed: errors.length === 0 && dirtyPaths === 0 && pendingMergeBack === 0 && unaccounted === 0,
    counts: {
...
export function summarizeWorktreeAudit(audit) {
  ...
  for (const entry of audit.worktrees) {
    lines.push(`worktree: ${entry.path} ${entry.branch ?? '(detached)'}${entry.claimed ? '' : ' [unmanaged]'}${entry.head ? ` @${String(entry.head).slice(0, 12)}` : ''}`);
  }
