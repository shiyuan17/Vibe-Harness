...
- 本地开发、调试和临时运行日志写入 `.loopengine/log/`，例如 `vite-dev.out/err`；`.loopengine/artifacts/` 只用于截图、trace、导出文件等交付证据产物。
...
```bash
rg '"correlationId":"req-123"' .loopengine/log/
rg '"level":"error"' .loopengine/log/ | rg '"component":"orders"'
rg 'ORDER_CREATE_FAILED|req-123' .loopengine/log/
```
