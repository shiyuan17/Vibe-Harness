...
> 完成 Plan 时填写一行：直接实施（0–1 个软信号）/ 拆分实施（硬触发或 2–3 个软信号及依据）/ 拆分并声明依赖（4+ 个软信号及依赖摘要）。该判定不构成写入、提交、推送或外部操作授权。

## 实施任务拆分（仅判定为拆分时填写）

> 计划拆分判定命中拆分档时填写；每个任务须可独立执行、独立验证、独立提交。该表不由 Vibe-Harness 解析或作为完成门禁。
